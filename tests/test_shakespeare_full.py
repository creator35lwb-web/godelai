...
def main():
    """Run Shakespeare benchmark on FULL Tiny Shakespeare dataset."""
    print("=" * 70)
    print("📚 GODELAI FULL TINY SHAKESPEARE BENCHMARK (v1.1.0)")
    print("=" * 70)

    # Configuration - Optimized for full 1.1MB dataset
    config = {
        "seq_length": 100,
        "batch_size": 64,  # Larger batches for full dataset
        "embedding_dim": 128,  # Increased capacity
        "hidden_dim": 256,  # Increased capacity
        "num_layers": 2,
        "epochs": 30,  # More epochs for full dataset
        "learning_rate": 0.002,
    ...
    for epoch in range(config["epochs"]):
        ...
        # Compute T-Score on a small sample (expensive, so only on 5 batches for full dataset)
        print(f"   Computing T-Score (wisdom metric)...")
        avg_t_score, sleep_events = compute_epoch_t_score(
            model, agent, train_batches, criterion, num_batches=5
        )
